...
export default {
  async createLayer(requirementsPath: string): Promise<string> {
    ...
    await $`docker run --rm --platform linux/amd64 -v ${process.cwd()}/${outputDir}:/output ${dockerImage} \
      bash -c "pip install -r /output/requirements.txt --target /output/python --no-cache-dir"`;
    
    console.log("Creating zip file...");
    await $`cd ${outputDir} && zip -r ${zipName} python`;
    
    // Cleanup
    await $`rm ${outputDir}/requirements.txt`;
    await $`rm -rf ${layerDir}`;
